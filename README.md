...
온디바이스 음성 인식 파이프라인을 Unity 3D 전투 게임 환경에 적용한 AI·SW 캡스톤디자인2 프로젝트입니다.  
Whisper API를 사용하여 한국어 음성 명령을 인식하고, 인식된 명령을 게임 내 스킬 입력으로 변환합니다.
...
- R 키 기반 음성 스킬 입력
- Whisper API 기반 한국어 STT 처리
- 온디바이스 음성 인식 파이프라인 기반 인터페이스 구조
...
| 이름 | 역할 |
|---|---|
| 안정빈 | 오디오·BGM 작곡 및 믹싱, 3D 에셋 제작(AI 툴 활용) |
| 원용준 | 3D 레벨 디자인 및 맵 환경 구축, 에셋 라이브러리 전반 관리, 물리 충돌 로직 최적화 |
| 김민우 | VFX 스킬 연출 구현, 시스템 최적화, Whisper API 연동 |
...
| `Assets/StreamingAssets` | Whisper 모델 파일 배치 위치 |
...
Whisper API를 한국어로 사용하기 때문에, 짧고 명확한 한국어 명령어를 기준으로 스킬 명령을 구성했습니다.
...
측정 결과, 고성능 PC 환경인 Desktop A에서 가장 낮은 평균 지연 시간이 나타났고, 저전력 기반 환경인 Laptop C에서 가장 높은 지연 시간이 나타났습니다. 이를 통해 Whisper API 기반 STT 처리 시간은 하드웨어 성능과 실행 환경의 영향을 크게 받을 수 있음을 확인했습니다.
